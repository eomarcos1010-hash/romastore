...
let products = load(PRODUCTS_KEY, defaultProducts);
let categories = load(CATEGORIES_KEY, defaultCategories);
...
function showSection(section) {
  ...
  document.querySelectorAll(".dashboard-section").forEach(item => {
...
function updateOverview() {
  $("overviewCategories").textContent = categories.length;
...
function setupStoreForm() {
  $("heroImage").addEventListener("change", async event => {
    ...
    try {
      ...
      showToast("Não foi possível carregar o banner.");
    }
  });
  ...
  $("storeForm").addEventListener("submit", async event => {
    ...
    if (file) {
      config.heroImage = await readFileAsDataURL(file);
    }

    config.storeName = $("storeName").value.trim();
    config.storeDescription = $("storeDescription").value.trim();
...
function loadStoreForm() {
  ...
  $("storeDescription").value = config.storeDescription || "";

  $("heroPreview").src = config.heroImage || "";
  $("heroPreview").style.display = config.heroImage ? "block" : "none";
}
...
function setupCategoryForm() {
  ...
  $("categoryImage").addEventListener("change", async event => {
    const file = event.target.files[0];
    ...
    try {
      ...
      $("categoryPreview").src = image;
      $("categoryPreview").style.display = "block";
    } catch {
      showToast("Não foi possível carregar a imagem da categoria.");
    }
  });
  ...
  $("categoryForm").addEventListener("submit", async event => {
    ...
    const id = normalizeId($("categorySlug").value.trim());
    const oldId = $("categoryId").value;
    const file = $("categoryImage").files[0];

    if (!name || !id) {
      showToast("Preencha o nome e o identificador.");
      return;
    }

    const alreadyExists = categories.some(category => {
      return category.id === id && category.id !== oldId;
    });

    if (alreadyExists) {
      showToast("Esse identificador já existe.");
      return;
    }
...
function setupProductForm() {
  ...
  $("productImage").addEventListener("change", async event => {
    ...
    try {
      ...
      showToast("Não foi possível carregar a imagem do produto.");
    }
  });
  ...
  $("productForm").addEventListener("submit", async event => {
    ...
    const mainImageFile = $("productImage").files[0];
    const additionalFiles = Array.from(
      $("productImages").files
    );

    if (!name || !category) {
      showToast("Preencha o nome e a categoria.");
      return;
    }

    const existingProduct = products.find(
      product => Number(product.id) === Number(idValue)
    );

    let image = existingProduct?.image || "";

    if (mainImageFile) {
      image = await readFileAsDataURL(mainImageFile);
    }
    ...
    if (additionalFiles.length) {
      images = await Promise.all(
        additionalFiles.map(file => readFileAsDataURL(file))
      );
    }
...
function openProductForm(product = null) {
  ...
  if (product) {
    ...
    $("productOldPrice").value = product.oldPrice || 0;
    $("productRating").value = product.rating || "5.0";
...
function setupSupportForm() {
  $("supportForm").addEventListener("submit", event => {
    ...
    config.discord = $("discord").value.trim();
    config.whatsapp = $("whatsapp").value.trim();
    config.tiktok = $("tiktok").value.trim();
    config.instagram = $("instagram").value.trim();

    save(CONFIG_KEY, config);

    showToast("Links de suporte salvos.");
  });
}
